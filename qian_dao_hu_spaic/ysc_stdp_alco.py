...
audio_lock = FileLock(LOCK_FILE_PATH, timeout=5) # 这里要大于 大模型录音的总体时长
...
class Gouzi:
    ...
    def __init__(self) -> None:
        ...
        self.state_queue = deque(maxlen=self.emo_len) # 与emo_queue使用同一把锁

        # 这个queue 其实除了放进去 emotion 还可以放进去 当时的状态输入，因此的话，在决策线程里面，就可以根据此时的 情感 加上决策的输入
        # 进行 同一的影响了， 或者就是 再拿一个 queue， 但是使用同一把锁， 也可以i获得状态的输入
    ...
    def start(self):
        ...
        self.robot_net.load_weight_and_buffer(model_path="save_600/real_ysc_model_64", buffer_path='real_ysc_buffer_600_64.pth') # 这里以后还可以换成 其他训练轮数的模型
    ...
    def action_handle_thread(self):
        # self.controller.thread_active = False # 先管道
        ...
        while True:
            with self.emo_queue_lock:
                if self.emo_queue.count(EMO['POSITIVE']) == self.emo_len:
                    ...
                    self.is_moving = True
                    #########################################
                    if self.state_queue[0][6]  == 1: # 有酒精
                            
                        # self.controller.hou_tui_2s()
                        # self.controller.light_eyes()
                    ...
                    elif self.state_queue[0][4] == 1: # 红衣服：
                    ...
                    """ self.emo_queue.popleft()
                    self.state_queue.popleft() """
                    self.emo_queue.clear() # 队列清空 防止 多次输入
                    self.state_queue.clear()
                    self.is_moving = False
                    #
                    ########################################
            time.sleep(0.5)
                # self.last_emotion = self.temp_emotion # 暂时并不使用
    ...
    def emo_handle_thread(self):
        ...
        while True:
            ...
            if self.is_moving: # 如果正在运动，也是不进行前向传播
                self.clear()
                continue
            elif sum(temp_input) < 1: # 这里其实也可以 不是1 ？？ 增大一点的话会不会 就 更真实了？？ 也就是有多个输入进去
            ...
            self.clear() # 清除状态 ，这个清除到底要是在哪里比较合适 ，可能还要再想一想
            # 
            ...
            #  这时最耗时的行为吗
            real_label = self.robot_net.new_check_label_from_data(temp_input) # 也可以去掉
            print("get in 0??")
            temp_predict = self.robot_net.ysc_pretrain_step_and_predict(data=temp_input, reward=0) # 返回预测结果 # 暂时不让有变化
            print("get in 1??")
            self.emo_state_queue_add_in_lock(emo_data=temp_predict, input_data=temp_input[0][0:input_node_num_origin]) # 这里加入情感和状态
            print("get in 2??")
    ...
    def emo_state_queue_add_in_lock(self, emo_data, input_data):
        with self.emo_queue_lock:
            self.emo_queue.append(emo_data)
            self.state_queue.append(input_data)
